...
async function main() {
    ...
    let opts = {
        ...
        initialState: {
            messages: [
                { sender: 'block', message: 'secure sees' },
                { sender: 'block', message: 'on the blockchain' },
                { sender: 'block', message: 'endless uses' }
            ]
        }
    };
    if (dev) {
     ...
     * type string and that the message is around 50 characters (so the sees doesn't get spammed)
     *
...
process.on('unhandledRejection', function(reason, p){
    ...
    console.log(
        ...
        https://github.com/block/blocksees
        `
    )
